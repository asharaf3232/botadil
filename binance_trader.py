# -*- coding: utf-8 -*-
# =======================================================================================
# --- 💣 بوت كاسحة الألغام (Minesweeper Bot) v2.2 (Final Stability Patch) 💣 ---
# =======================================================================================
# - [إصلاح حرج] حل مشكلة انهيار `TypeError` في دالة الفحص بسبب عدم توافق
#   أنواع بيانات الوقت، مما يعيد الاستقرار الكامل للبوت.
# - [إصلاح حرج] إصلاح خطأ منطقي حيث لم تكن قيمة RVOL تُمرر إلى الاستراتيجيات،
#   مما يعيد تفعيل الفلتر ويحسن جودة الإشارات.
# - [ميزة جديدة] إضافة آلية آمنة لإعادة تعيين قاعدة البيانات عند الطلب عن طريق
#   إنشاء ملف `_reset_db_please.txt`.
# =======================================================================================
    ...
    while not queue.empty():
        ...
        try:
            ...
            for scanner_name in settings['active_scanners']:
                ...
                # [إصلاح حرج] تمرير قيمة RVOL الصحيحة إلى جميع الاستراتيجيات
                if asyncio.iscoroutinefunction(scanner_func):
                    result = await scanner_func(df.copy(), settings, rvol, adx_value, exchange, symbol)
                else:
                    result = scanner_func(df.copy(), settings, rvol, adx_value, exchange, symbol)
...
async def perform_scan(context: ContextTypes.DEFAULT_TYPE):
    async with scan_lock:
        ...
        # [إصلاح حرج] استخدام كائنات datetime متوافقة لحساب المدة
...
async def track_open_trades(context: ContextTypes.DEFAULT_TYPE):
    ...
    for trade in active_trades:
        ...
        try:
            ...
            # [إصلاح حرج] التحقق من أن القيم الرقمية ليست None قبل المقارنة
...
async def button_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("dashboard_") and data.endswith(('_all', '_real', '_virtual')):
        # [إصلاح حرج] إعادة هيكلة هذا الجزء لمنع الأخطاء
        try:
            parts = data.split('_')
            report_type = parts[1]
...
async def post_init(application: Application):
    ...
    await application.bot.send_message(chat_id=TELEGRAM_CHAT_ID, text=f"🚀 *بوت كاسحة الألغام (v1.9) جاهز للعمل!*", parse_mode=ParseMode.MARKDOWN)
...
def main():
    # [جديد] إضافة آلية إعادة تعيين قاعدة البيانات
    ...
    print("🚀 Starting Minesweeper Bot v1.9 (Stability & Hotfix)...")
