# -*- coding: utf-8 -*-
# =======================================================================================
# --- 💣 بوت كاسحة الألغام (Minesweeper Bot) v2.3 (Radical Fix Release) 💣 ---
# =======================================================================================
# - [إصلاح جذري] إعادة كتابة دالة معالجة الأزرار بالكامل لفصل المهام ومنع الأخطاء.
#   - دالة `main_button_handler` الآن مسؤولة فقط عن عرض فلاتر التقارير.
#   - دالة `report_filter_handler` جديدة ومخصصة لاستدعاء التقارير وعرضها.
# - [نتيجة] حل نهائي لمشكلة انهيار التقارير `UnboundLocalError`.
# - [تأكيد] الإبقاء على جميع إصلاحات v2.2 (استقرار الفحص، RVOL، إعادة تعيين DB).
# =======================================================================================
        ...
        try:
            ...
            for scanner_name in settings['active_scanners']:
                if scanner_func := SCANNERS.get(scanner_name):
                    # [FIX] Pass the correct `rvol` value to scanners
...
async def perform_scan(context: ContextTypes.DEFAULT_TYPE):
    async with scan_lock:
        ...
        scan_duration = 0
        if status['last_scan_start_time'] != 'N/A':
            try:
                # [FIX] Handle timezone difference correctly
                start_time_naive = datetime.strptime(status['last_scan_start_time'], '%Y-%m-%d %H:%M:%S')
                start_time_aware = EGYPT_TZ.localize(start_time_naive)
                scan_duration = (datetime.now(EGYPT_TZ) - start_time_aware).total_seconds()
            except Exception as e:
                logger.error(f"Could not calculate scan duration: {e}")
...
async def close_trade_in_db(context: ContextTypes.DEFAULT_TYPE, trade: dict, exit_price: float, status: str):
    ...
    start_time_naive = datetime.strptime(trade['timestamp'], '%Y-%m-%d %H:%M:%S')
    start_time_aware = EGYPT_TZ.localize(start_time_naive)
    duration = datetime.now(EGYPT_TZ) - start_time_aware
...
# [REWRITTEN] Handles main dashboard buttons and other simple callbacks
async def main_button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("dashboard_"):
        ...
        if action in ["stats", "active_trades", "strategy_report"]:
            keyboard = InlineKeyboardMarkup([
                [InlineKeyboardButton("📊 الكل", callback_data=f"filter_{action}_all")],
                [InlineKeyboardButton("📈 حقيقي فقط", callback_data=f"filter_{action}_real"), InlineKeyboardButton("📉 وهمي فقط", callback_data=f"filter_{action}_virtual")],
        ...
        # For direct reports without filters
        report_string, keyboard = f"⏳ جارِ إعداد تقرير: *{action.replace('_',' ')}*...", InlineKeyboardMarkup([[InlineKeyboardButton("🔙 العودة للوحة التحكم", callback_data="dashboard_refresh")]])
        await query.edit_message_text(report_string, parse_mode=ParseMode.MARKDOWN)

        if action == "debug": report_string = await generate_debug_report_string(context)
...
# [NEW] Handles the report filter buttons specifically
async def report_filter_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query; await query.answer(); data = query.data
    back_to_dashboard_keyboard = InlineKeyboardMarkup([[InlineKeyboardButton("🔙 العودة للوحة التحكم", callback_data="dashboard_refresh")]])
    
    try:
        _, report_type, trade_mode_filter = data.split('_')
        ...
        logger.error(f"Error in report_filter_handler: {e}", exc_info=True)
        await query.edit_message_text("❌ حدث خطأ أثناء إعداد التقرير.", reply_markup=back_to_dashboard_keyboard)
...
async def generate_debug_report_string(context: ContextTypes.DEFAULT_TYPE):
    ...
    if status['last_scan_end_time'] != 'N/A' and status['last_scan_start_time'] != 'N/A':
        start_time_naive = datetime.strptime(status['last_scan_start_time'], '%Y-%m-%d %H:%M:%S')
        start_time_aware = EGYPT_TZ.localize(start_time_naive)
        end_time_naive = datetime.strptime(status['last_scan_end_time'], '%Y-%m-%d %H:%M:%S')
        end_time_aware = EGYPT_TZ.localize(end_time_naive)
        scan_duration = f"{(end_time_aware - start_time_aware).total_seconds():.0f} ثانية"
...
async def post_init(application: Application):
    ...
    await application.bot.send_message(chat_id=TELEGRAM_CHAT_ID, text=f"🚀 *بوت كاسحة الألغام (v2.3) جاهز للعمل!*", parse_mode=ParseMode.MARKDOWN)
...
def main():
    # --- [NEW] Database Reset Feature ---
    reset_file = os.path.join(APP_ROOT, '_reset_db_please.txt')
    if os.path.exists(reset_file):
        logger.warning("Reset file found! Deleting database and starting fresh.")
        ...
        try:
            ...
            logger.info(f"Removed reset file: {reset_file}")
        except OSError as e:
            logger.error(f"Error removing reset file: {e}")

    print("🚀 Starting Minesweeper Bot v2.3 (Radical Fix Release)...")
    ...
    # [REWRITTEN] Split button handlers for stability
    app.add_handler(CallbackQueryHandler(report_filter_handler, pattern="^filter_"))
    ...
    app.add_handler(CallbackQueryHandler(main_button_handler)) # Handles all other buttons
