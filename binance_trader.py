...
async def initialize_exchanges():
    async def connect(ex_id):
        ...
        # [تعديل] إضافة مفاتيح API عند الاتصال بـ Binance
        if ex_id == 'binance' and BINANCE_API_KEY != 'YOUR_BINANCE_API_KEY':
            logger.info("Binance API Keys found. Initializing with credentials.")
        ...
        for signal in signals:
            ...
            if is_real_trade:
                ...
                if real_trade_result:
                    # تحديث الإشارة بالبيانات الحقيقية (الكمية، IDs)
                ...
                # --- منطق التداول التجريبي (الافتراضي) ---
...
async def track_open_trades(context: ContextTypes.DEFAULT_TYPE):
    ...
    async def check_trade(trade):
        ...
        try:
            ...
            # [إصلاح] استعادة المنطق الكامل لمتابعة الصفقات التجريبية
...
async def check_market_regime():
    ...
    if settings.get("fear_and_greed_filter_enabled", True):
        # [تعديل] استدعاء الدالة غير المتزامنة الجديدة
...
async def button_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # [ميزة جديدة] بدء محادثات الأدوات
...
# [ميزة جديدة] معالج أزرار الأدوات الأخرى
    ...
    if active_tool:
        ...
        if state == 'awaiting_symbol':
            ...
            if active_tool != 'manual_trade':
                user_data.pop(active_tool, None) # إنهاء المحادثة
            return
...
# [ميزة جديدة] أوامر بدء محادثات الأدوات
...
async def fetch_and_display_balance(exchange_id, query):
    ...
    try:
        ...
        # جلب الأسعار الحالية لتقييم الرصيد
        ...
        for currency, amount in total_balance.items():
            if amount > 0:
                ...
                if usdt_value > 1: # عرض الأرصدة التي تزيد عن 1 دولار فقط
                    assets.append({'currency': currency, 'amount': amount, 'usdt_value': usdt_value})
        
        # ترتيب الأصول حسب القيمة
        ...
        for asset in assets[:15]: # عرض أكبر 15 رصيدًا
