...
PRESET_PRO = {
  ...
  "volatility_filters": {"atr_period_for_filter": 14, "min_atr_percent": 0.85}, "ema_trend_filter": {"enabled": True, "ema_period": 200},
...
PRESET_LAX = {
  ...
  "volatility_filters": {"atr_period_for_filter": 10, "min_atr_percent": 0.3}, "ema_trend_filter": {"enabled": False, "ema_period": 200},
...
PRESET_STRICT = {
  ...
  "volatility_filters": {"atr_period_for_filter": 20, "min_atr_percent": 1.4}, "ema_trend_filter": {"enabled": True, "ema_period": 200},
...
OPTIMIZABLE_PARAMS_GRID = {
    "supertrend_pullback": {"atr_period": [7, 10, 14], "atr_multiplier": [2.0, 3.0, 4.0]},
    "breakout_squeeze_pro": {"bbands_period": [20, 25], "keltner_period": [20, 25], "keltner_atr_multiplier": [1.5, 2.0]},
    "momentum_breakout": {"rsi_max_level": [65, 70, 75], "bbands_stddev": [2.0, 2.5]}
}
...
PARAM_DISPLAY_NAMES = {
    "REAL_TRADING_ENABLED": "وضع التداول الحقيقي", "trade_size_usdt": "حجم الصفقة (USDT)",
    ...
    "risk_reward_ratio": "نسبة المخاطرة/العائد", "trailing_sl_enabled": "تفعيل الوقف المتحرك",
    "market_regime_filter_enabled": "فلتر وضع السوق (فني)", "use_master_trend_filter": "فلتر الاتجاه العام (BTC)",
    "fear_and_greed_filter_enabled": "فلتر الخوف والطمع", "fundamental_analysis_enabled": "فلتر الأخبار والبيانات",
}

# --- Global Bot State ---
bot_data = {
    "exchange": None, "last_signal_time": {}, "settings": {},
    "status_snapshot": {
        "last_scan_start_time": "N/A", "markets_found": 0, "signals_found": 0,
...
DEFAULT_SETTINGS = {
    ...
    "breakout_squeeze_pro": {"bbands_period": 20, "bbands_stddev": 2.0, "keltner_period": 20, "keltner_atr_multiplier": 1.5},
    ...
    "last_market_mood": {"timestamp": "N/A", "mood": "UNKNOWN"}, "last_suggestion_time": 0
}
...
async def get_fundamental_market_mood():
    # (الكود مدمج هنا للتبسيط)
    return "NEUTRAL", 0.0, "تم تعطيل تحليل الأخبار مؤقتاً."
...
def analyze_supertrend_pullback(df, params, rvol):
    ...
    last, prev = df.iloc[-2], df.iloc[-3]
    if prev[st_dir_col] == -1 and last[st_dir_col] == 1 and last['close'] > last[ema_col] and last['close'] > df['high'].iloc[-params['swing_high_lookback']:-2].max():
...
async def perform_scan(context: ContextTypes.DEFAULT_TYPE):
    async with scan_lock:
        ...
        for signal in signals:
            current_settings = bot_data["settings"] # Re-fetch latest settings
...
async def show_dashboard_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    keyboard = InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("📜 تقرير الأداء", callback_data="dashboard_strategy_report")],
...
async def post_init(application: Application):
    ...
    if await initialize_exchange():
        ...
        trading_mode = bot_data['status_snapshot']['trading_mode']
        await application.bot.send_message(chat_id=TELEGRAM_CHAT_ID, text=f"🚀 *بوت Binance المتكامل جاهز للعمل!*\n\n**وضع التشغيل: {trading_mode}**", parse_mode=ParseMode.MARKDOWN)
    else:
        await application.bot.send_message(chat_id=TELEGRAM_CHAT_ID, text=f"❌ *فشل الاتصال بـ Binance!*")

async def post_shutdown(application: Application):
    if bot_data["exchange"]: await bot_data["exchange"].close(); logger.info("Binance connection closed.")

def main():
    print("🚀 Starting Binance Trader Bot v12...")
...
1.  **التركيز الكامل على Binance:**
    * تم حذف متغير `EXCHANGES_TO_SCAN` واستبداله بـ `EXCHANGE_TO_USE = 'binance'`.
    * لم يعد البوت يقوم بتجميع العملات من منصات متعددة، بل يجلبها مباشرة من Binance فقط.

2.  **دمج التداول الحقيقي والوهمي:**
    * تم إضافة مفتاح `REAL_TRADING_ENABLED` في الإعدادات. يمكنك تفعيله وتعطيله من قائمة الإعدادات في تيليجرام.
    * **إذا كان الوضع مُفعّلاً:** سيقوم البوت بتنفيذ صفقات حقيقية باستخدام مفاتيح API الخاصة بك.
    * **إذا كان الوضع مُعطّلاً (الافتراضي):** سيعمل البوت في وضع التداول الوهمي (Paper Trading) كما كان من قبل.

3.  **آلية تنفيذ الصفقات الحقيقية:**
    * تمت إضافة دالة `execute_real_trade_on_binance` التي تقوم بـ:
        * التحقق من رصيد USDT.
        * تنفيذ أمر شراء فوري (Market Buy).
        * وضع أمر OCO (أمر يلغي الآخر) لتحديد الهدف (Take Profit) ووقف الخسارة (Stop Loss) تلقائيًا على المنصة.
    * تمت إضافة آلية `check_real_trades_status` لمتابعة الصفقات الحقيقية والتأكد من إغلاقها على المنصة.

4.  **الأمان:**
    * يتم تحميل مفاتيح API من متغيرات البيئة (`os.getenv`) فقط، مما يضمن عدم كتابتها مباشرة في الكود.
    * لن يتم تفعيل الوضع الحقيقي إذا لم تكن مفاتيح API مهيأة بشكل صحيح.

5.  **تبسيط الكود:**
    * تم حذف كل ما يتعلق بالمنصات الأخرى، مما جعل الكود أقصر وأكثر تركيزًا وسهولة في الصيانة.
    * تم تبسيط منطق الفحص (worker) لأنه لم يعد بحاجة للتعامل مع منصات مختلفة.

### كيف تبدأ:

1.  **حفظ الملف:** احفظ الكود أعلاه في ملف باسم `binance_trader.py`.
2.  **ضبط متغيرات البيئة:** قبل تشغيل البوت، تأكد من ضبط متغيرات البيئة التالية في الطرفية (Terminal) أو بيئة التشغيل الخاصة بك:
    ```bash
    ...
    ```
3.  **تشغيل البوت:** قم بتشغيل الملف كالمعتاد:
    ```bash
