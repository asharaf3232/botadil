...
async def main():
    ...
    # [IMPROVEMENT] إضافة مهمة تتبع الصفقات
    job_queue.run_repeating(
        track_active_trades,
    ...
    try:
        ...
        # تنظيف المصادر عند إيقاف البوت
        logger.info("🧹 Cleaning up resources...")
        for exchange in bot_data["exchanges"].values():
            if exchange:
                try:
                    await exchange.close()
                    logger.info(f"Closed connection to {exchange.id}")
                except Exception as e:
...
if __name__ == "__main__":
    ...
    # <-- [FIX] The robust restart loop
    while True:
        try:
            asyncio.run(main())
            # If main() finishes without an exception, break the loop
            break
        except KeyboardInterrupt:
            print("\n🛑 Bot stopped by user (Ctrl+C).")
            break
        except Exception as e:
            # Log the critical failure and wait before restarting
            print(f"\n💥 CRITICAL FAILURE IN MAIN EXECUTION: {e}")
            print(f"🔄 The bot will restart in 15 seconds...")
            time.sleep(15)
